...
public struct Article: Codable {
    ...
    public var part: Part? {
        get {
            ...
            return try? Part(erpCode: _part)
        }
        set { _part = newValue?.erpCode }
    }
    private(set) var _quality: String?
    public var quality: Quality? {
        get {
            guard let _quality else { return nil }
            return try? Quality(erpCode: _quality)
        }
        set { _quality = newValue?.erpCode }
    }
    public var length: Float?
    
    fileprivate init(itemId: String, part: Part?, quality: Quality?, length: Float?) {
        self.itemId = itemId
        self._part = part?.erpCode
        self.part = part
        self._quality = quality?.erpCode
